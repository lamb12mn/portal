<template>
  <div>
    <!-- 页面标题区域 - 增加科技感背景和动态元素 -->
    <section
      class="relative overflow-hidden bg-gradient-to-r from-primary-700 to-primary-500 py-20 md:py-24"
    >
      <!-- 科技感网格背景 -->
      ...
      <div class="container-custom relative z-10">
        ...
        <div class="mb-6">
          <h1 class="text-white flex items-center flex-wrap">
            <span v-if="locale === 'zh-CN'" class="title-first-part mr-3"
              >我们的</span
            >
            ...
            <span class="relative inline-block title-highlight">
              <!-- 背景效果 -->
              <span
                class="absolute inset-0 bg-white/15 backdrop-blur-sm rounded-lg transform rotate-0 border border-white/20 title-bg-animate"
              ></span>

              <!-- 微妙的装饰光点 -->
              <span
                class="absolute -top-1 -right-1 w-2.5 h-2.5 bg-white rounded-full opacity-70 animate-pulse"
              ></span>
              <span
                class="absolute -bottom-1 -left-1 w-2 h-2 bg-white rounded-full opacity-60"
                style="animation: pulse 2s infinite; animation-delay: 0.5s"
              ></span>
              ...
              <!-- 底部装饰线 -->
              <span
                class="absolute bottom-0 left-0 right-0 h-1.5 bg-gradient-to-r from-secondary-300 to-primary-200 rounded-full title-underline-animate"
              ></span>
            </span>
          </h1>
        </div>
    ...
    <!-- 产品筛选 - 增强视觉吸引力 -->
    <section
      class="py-8 bg-gradient-to-b from-gray-50 to-white border-b border-gray-200 relative overflow-hidden"
    >
      <!-- 装饰背景元素 -->
      <div
      ...
      ></div>
      <div
        class="absolute -bottom-10 -left-10 w-32 h-32 bg-secondary-50 rounded-full opacity-30"
      ></div>
        ...
        <div class="relative text-center mb-6">
          ...
          >
            ...
            <div class="flex items-center justify-center">
              ...
              ></div>
              <span class="text-primary-700 font-semibold tracking-wide">{{
                t("products.filter.title")
              }}</span>
              <div
                class="w-1.5 h-1.5 rounded-full bg-primary-500 ml-2 animate-pulse"
        ...
        <div class="flex flex-wrap items-center justify-center gap-3 mt-6">
          ...
          >
            ...
            <span
              ...
              class="absolute inset-0 bg-gradient-to-r from-primary-600 to-primary-400 opacity-50"
            ></span>
            <!-- 选中项的闪光效果 -->
            <span
              v-if="activeCategory === category.id"
              class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-all duration-1000 transform"
            ></span>
    ...
    <section class="section bg-white relative overflow-hidden">
      ...
      ></div>
      <div
        class="absolute -bottom-40 -left-40 w-96 h-96 bg-secondary-50 rounded-full opacity-50"
      ></div>
      ...
      <div class="container-custom relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            <!-- 卡片主体 -->
            <div
              class="relative flex flex-col h-full bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-primary-100 cursor-pointer"
            ...
            >
              ...
              <div class="p-6 flex flex-col flex-grow">
                <h3
                  class="text-xl font-bold mb-3 group-hover:text-primary-600 transition-colors"
                >
                  {{ product.content}}
                </h3>
                ...
                >
                  <div class="flex items-center">
                    <span
                      v-if="product.isFree"
                      class="flex items-center text-green-600 font-medium bg-green-50 px-3 py-1.5 rounded-full text-xs"
                    >
                      <svg
                    ...
                    <span
                      v-else
                      class="flex items-center text-primary-600 font-medium bg-primary-50 px-3 py-1.5 rounded-full text-xs"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-3 w-3 mr-1"
                  ...
                  >
                    ...
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-4 w-4 ml-1.5 group-hover:translate-x-1 transition-transform duration-200"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
    ...
    <section class="section bg-gradient-tech relative overflow-hidden">
      ...
      <div class="container-custom relative z-10 py-12">
        <div
          class="card-glass relative overflow-hidden bg-white/10 backdrop-blur-md p-8 md:p-12 text-center border border-white/20 rounded-2xl shadow-lg"
        >
          <div
          ...
          <h2 class="mb-6 text-white">
            {{ t("products.customService.title") }}
          </h2>
...
const products = [
  ...
  return {
    ...
    description: hasTranslation(
      `products.productItems.${product.id}.description`
    )
...
const allProductsMap = reactive(new Map());
const allProducts = ref([])
// const allProducts = computed(() => {
//     let all = [];
//   // 遍历 productsList，将每个产品的 products 存储到 Map 中
//     productsList.forEach(product => {
//     allProductsMap.set(product.id, product.products)
//         all = [...all, ...product.products];
//     });
// });
// 过滤产品
const filteredProducts = computed(() => {
    return allProductsMap.get(activeCategory.value) || [];
});

// 获取分类名称
const getCategoryName = (categoryId) => {
  return categoryId
    ? t(`products.filter.categories.${categoryId}`)
    : t("products.filter.categories.other");
};
...
onBeforeMount(() => {
    let navs = [], allItems = productsList
  .flatMap(product => product.products || [])
  .flatMap(subProduct => (subProduct.items.map(item => {return {...item, productId: subProduct.id, productName: subProduct.name}})) || []);
    productsList.forEach(product => {
        const { id, title, products } = product;
        navs.push({
            id,
            title,
        })
        let allItems = products.flatMap(subProduct => (subProduct.items.map(item => {return {...item, productId: subProduct.id, productName: subProduct.name}})) || [])
        allProductsMap.set(product.id, allItems)
    });
    allProductsMap.set('all', allItems)
  navList.value = [
    {
      id: "all",
      title: "全部",
